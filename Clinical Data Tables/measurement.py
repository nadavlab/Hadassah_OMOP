...
# TODO
# LABORATORY = 0  # טיפול מיוחד בילוד
...
for index_row, row in source_table.iterrows():
    ...
    if row[7] == "" or pd.isna(row[7]):
        ...
        measurement_datetime = datetime.strptime(
            match_visit_occurrence['visit_start_datetime'].values[0], '%Y-%m-%d %H:%M:%S')
    ...
    measurement_type_concept_id = 0  # unit , where the Measurement record was recorded
...
for index_row, row in source_table.iterrows():
    ...
    measurement_type_concept_id = 0  # NEONATAL_INTENSIVE_CARE_UNIT  # טיפול נמרץ ילודים
...
for index_row, row in source_table.iterrows():
    ...
    measurement_type_concept_id = 0  # NEWBORN_NURSERY_UNIT

    value_as_concept_id = ""
    value_as_number = ""

    if row[5] == 'low':
        value_as_concept_id = 0  # 4083207  # Below reference range
    elif row[5] == 'high':
        value_as_concept_id = 0  # 4084765  # Above reference range
    # else:
