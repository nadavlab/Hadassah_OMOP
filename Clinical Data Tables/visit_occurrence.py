...
df_table_92 = pd.DataFrame(table_92, columns=[
                           "ID_BAZNAT", "Event_baznat", "Department", "Department_Entry_Date", "Department_Exit_Date"])
...
df_care_site = pd.DataFrame(care_site_table, columns=[
...
for index_row, row in source_table.iterrows():
    ...
    match_event_baznat_92 = df_table_92.loc[df_table_92['Event_baznat']
                                            == row['Event_baznat']]

    no_date_flag_start = True if type(row["HOSP_ENTRY_DATE"]) != str and math.isnan(
        row["HOSP_ENTRY_DATE"]) else False
    no_date_flag_end = True if type(row["HOSP_EXIT_DATE"]) != str and math.isnan(
        row["HOSP_EXIT_DATE"]) else False

    entry_date = ''
    exit_date = ''
    # handle miss date
    if no_date_flag_start == True or no_date_flag_end == True:  # miss entry date & exit date ####
        entry_date_table_92 = True if match_event_baznat_92[
            "Department_Entry_Date"].size > 0 else False
        ...
        if no_date_flag_start and no_date_flag_start:
            if entry_date_table_92 and exit_date_table_92 and \
                ...
                entry_date = datetime.strptime(
                    match_event_baznat_92["Department_Entry_Date"].values[0], '%d/%m/%Y %H:%M')
                exit_date = datetime.strptime(
                    match_event_baznat_92["Department_Exit_Date"].values[0], '%d/%m/%Y %H:%M')
            ...
            while match_event_baznat_92["Department_Entry_Date"].size > index_date and entry_date is '':
                if match_event_baznat_92["Department_Exit_Date"].values[index_date] != '-1':
                    exit_date = datetime.strptime(
                        match_event_baznat_92["HOSP_EXIT_DATE"], '%d/%m/%Y %H:%M')
                    new_entry_date = datetime.strptime(
                        row['Department_Entry_Date'].values[index_date], '%d/%m/%Y %H:%M')
            ...
            while match_event_baznat_92["Department_Exit_Date"].size > index_date and exit_date is '':
                if match_event_baznat_92["Department_Exit_Date"].values[index_date] != '-1':
                    new_exit_date = datetime.strptime(
                        match_event_baznat_92["Department_Exit_Date"].values[index_date], '%d/%m/%Y %H:%M')
                    entry_date = datetime.strptime(
                        row['HOSP_ENTRY_DATE'], '%d/%m/%Y %H:%M')
    ...
    ### case -duplicates ###
    match_source_table = source_table.loc[source_table['Event_baznat']
                                          == row['Event_baznat']]
    ...
    if row['duplicates'] == True:  # case  - is duplicate -
        date_of_duplicate_id = duplicates_dic.get(
            original_id)  # the secned time or more
    ...
    if match_event_baznat_92["Department"].size > 0:
        ...
        match_care_site = df_care_site.loc[df_care_site["care_site_name"]
                                           == care_site_name]
    ...
    data.append([visit_occurrence_id, person_id, visit_concept_id, visit_start_date, visit_start_datetime, visit_end_date,
                 visit_end_datetime, visit_type_concept_id, provider_id, care_site_id, visit_source_value, visit_source_concept_id, admitted_from_concept_id,
...
df_result = pd.DataFrame(data, columns=['visit_occurrence_id', 'person_id', 'visit_concept_id',  'visit_start_date', 'visit_start_datetime', 'visit_end_date', 'visit_end_datetime',
                                        'visit_type_concept_id', 'provider_id', 'care_site_id', 'visit_source_value', 'visit_source_concept_id', 'admitted_from_concept_id', 'admitted_from_source_value',
